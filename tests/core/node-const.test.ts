...
import { WebSocketClient as ws } from 'eventnet/connectors';
import { createEndA } from 'helpers/e2e-helper';
...
describe('constant of Node', () => {
   ...
   test('Node in RemoteDomain', async () => {
      const endA = await createEndA(
         path.resolve(__dirname, './helpers/remote-node.js')
      );
      await endA.willReceive('ping');

      const domain8888 = domain(
         ws('ws://127.0.0.1:8888'),
         {
            key: 'test-password',
         }
      );
      await domain8888.ready();
      const nd = domain8888.nodes.nd1;
      expect(type(nd.uid)).toBe('number');
      expect(nd.type).toBe('Node');
      expect(nd.isSubnet).toBe(false);
      // nd.parent is not available
      expect(nd.domain).toBe(domain8888);
