import { BasicNodeMode, CallableElement } from '../../types';
import { assign } from '../../util/assign';
import { isValidArrayIndex } from '../../util/isValidArrayIndex';
import { debug, fulfilledPromise, hasPromise } from '../debug';
import { Line } from '../line';
import { Node } from '../node';
import { NodeStream } from '../stream';
import { defaultQueue, QueueScheduler } from './queueScheduler';

export abstract class BasicNode<T = any> extends Node<T | Promise<T>> {
  public abstract readonly ups: NodeStream;
  public abstract readonly dws: NodeStream;

  /**
   * 继承和实现
   *
   * T 为 BasicNode 的原本返回类型
   *
   * 二选一
   * 1. 用 _run 表示原本的逻辑， run 是根据 mode 执行相应的方法，相应的方法执行 _run ，需要将相应方法和 mode 设置为 public
   * 2. 用 run 表示真正的逻辑，废弃 _run 等等
   */

  public run(data?: any, caller?: Line): T | Promise<T> {
    switch (this.mode) {
      case BasicNodeMode.sync:
        return this._run!(data, caller);
      case BasicNodeMode.micro:
        return this.runMicro(data, caller);
      case BasicNodeMode.macro:
        return this.runMacro(data, caller);
      case BasicNodeMode.animationFrame:
        return this.runAnimationFrame(data, caller);
      default:
        return this
          .queue((this.mode as [BasicNodeMode, QueueScheduler | undefined])[1])
          .run(data, caller);
    }
  }
  protected _run?(data?: any, caller?: Line): T;

  protected mode?: BasicNodeMode | [BasicNodeMode, QueueScheduler | undefined];

  protected runSync(data?: any, caller?: Line): T {
    return this._run!(data, caller);
  }
  protected queue(
    queue = Array.isArray(this.mode) &&
      this.mode[0] === BasicNodeMode.queue &&
      this.mode[1]
      || defaultQueue,
  ) {
    return {
      run: (data?: any, caller?: Line) => {
        return new Promise<T>(resolve =>
          queue.schedule(() => {
            const result = this._run!(data, caller);
            resolve(result);
            return result;
          }));
      },
    };
  }
  protected runMicro(data?: any, caller?: Line): Promise<T> {
    return (fulfilledPromise as Promise<void>).then(() => this._run!(data, caller));
  }
  protected runMacro(data?: any, caller?: Line): Promise<T> {
    return new Promise(resolve => {
      // In fact, probably more than 4 milliseconds
      setTimeout(() => resolve(this._run!(data, caller)), 0);
    });
  }
  protected runAnimationFrame(data?: any, caller?: Line): Promise<T> {
    return new Promise(resolve => {
      requestAnimationFrame(() => resolve(this._run!(data, caller)));
    });
  }

  /**
   * This is provided for runtime that does not
   * support Promise and do not have a polyfill for Promise.
   */
  protected noret() {
    const fns = {
      queue: (queue = defaultQueue) => {
        return {
          run: (data?: any, caller?: Line) => {
            queue.schedule(() => {
              this._run!(data, caller);
            });
          },
        };
      },
      runMacro: (data?: any, caller?: Line) => {
        setTimeout(() => this._run!(data, caller), 0);
      },
      runAnimationFrame: (data?: any, caller?: Line) => {
        requestAnimationFrame(() => this._run!(data, caller));
      },
    };
    this.noret = () => fns;
    return fns;
  }

  public abstract readonly type: number;
  public generateIdentity(): { [field: string]: any } {
    return assign(super.generateIdentity(), { type: 'BasicNode' });
  }
  public abstract clone(): this;
}

function callableDws(line: Line, caller: Node, data?: any) {
  return line.run(data, caller);
}

export class BasicNodeDws {
  public readonly origin: NodeStream;
  constructor(thisStream: NodeStream) {
    this.origin = thisStream;
  }

  protected static readonly $x = (i: number, origin: NodeStream) => {
    const line = origin.get()[i];
    if (process.env.NODE_ENV !== 'production' && !line) {
      debug('BN_NonexistDws', origin.owner, new Error());
    }
    const f = callableDws.bind(null, line as Line, origin.owner);
    f.origin = line;
    return f as CallableElement;
  }

  public $0: CallableElement;
  public $1: CallableElement;
  public $2: CallableElement;
  public $3: CallableElement;
  public $4: CallableElement;
  public $5: CallableElement;
  public $6: CallableElement;
  public $7: CallableElement;
  public $8: CallableElement;
  public $9: CallableElement;

  public $(index: number, data?: any) {
    const f = BasicNodeDws.$x(index, this.origin);
    if (typeof data !== 'undefined') {
      f(data);
    }
    return f;
  }
  public all(data?: any) {
    this.origin.get().forEach(line =>
      line && line.run(data, this.origin.owner));
  }
  public allnerr(data?: any) {
    this.origin.get().filter(line => {
      if (!line) { return false; }
      if (!line.classes) { return true; }
      return !~line.classes.indexOf('error');
    }).forEach(line => (line as Line).run(data, this.origin.owner));
  }
  public id(id: string): CallableElement {
    const line = this.origin.getById(id);
    if (!line) {
      if (process.env.NODE_ENV !== 'production') {
        debug('BN_NonexistDws', this.origin.owner, new Error());
      }
    }
    const f = callableDws.bind(line, this.origin.owner);
    f.origin = line;
    return f;
  }
  public ask(
    askFor: string | string[] | ((line: Line | undefined) => boolean),
    data?: any,
  ) {
    const lines = this.origin.ask(askFor as any);
    const res: CallableElement[] = [];
    if (!lines.length) {
      return res;
    }
    lines.forEach(line => {
      if (typeof data !== 'undefined') { line.run(data, this.origin.owner); }
      const fn = (d => {
        line.run(d, this.origin.owner);
      }) as CallableElement;
      fn.origin = line;
      res.push(fn);
    });

    return res;
  }
  public dispense(id_value_or_index_value: { [index: number]: any } | { [id: string]: any }) {
    let downstream: Line | undefined;
    const isIndex = isValidArrayIndex(Object.keys(id_value_or_index_value)[0]);

    Object.keys(id_value_or_index_value).forEach(i => {
      downstream = (
        isIndex
          ? this.origin.get()[Number(i)]
          : this.origin.getById(i)
      ) as Line | undefined;

      if (downstream) {
        downstream.run((id_value_or_index_value as any)[i], this.origin.owner);
      } else if (process.env.NODE_ENV !== 'production') {
        debug('BN_NonexistDwsWarn', this.origin.owner);
      }
    });
  }
}

for (let i = 0; i < 10; ++i) {
  Object.defineProperty(BasicNodeDws.prototype, '$' + i, {
    get() {
      return (BasicNodeDws as any).$x(i, this.origin);
    },
    enumerable: true,
    configurable: true,
  });
}
