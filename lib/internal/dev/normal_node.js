...
class NormalNode {
    constructor(attrs, state, code) {
        ...
        for (const name of Object.keys(attrs)) {
            if (!eventnet_dev_1._attrsStore.typed[name] &&
                !eventnet_dev_1._attrsStore.normal[name].before &&
                !eventnet_dev_1._attrsStore.normal[name].after &&
                !eventnet_dev_1._attrsStore.normal[name].finish) {
                eventnet_dev_1._debug(`Node: Attribution '${name}' has not been installed.`);
            }
            if (eventnet_dev_1._attrsStore.typed[name] &&
                typeof attrs[name] !== eventnet_dev_1._attrsStore.typed[name]) {
                throw new Error(`EventNet.Node: The type of attribution '${name}' must be ${eventnet_dev_1._attrsStore.typed[name]}.`);
            }
        }
        this.code = code;
        ...
        this.sortAttrs();
        this.state = Object.assign({}, eventnet_dev_1.defaultState, state);
    ...
    sortAttrs() {
        ...
        for (const name in attr) {
            ...
            if (eventnet_dev_1._attrsStore.normal[name].before) {
                this._attrs.beforeSequence.push({
                    name,
                    value: attr[name],
                    priority: eventnet_dev_1._attrsStore.normal[name].beforePriority
                });
            }
            if (eventnet_dev_1._attrsStore.normal[name].after) {
                this._attrs.afterSequence.push({
                    name,
                    value: attr[name],
                    priority: eventnet_dev_1._attrsStore.normal[name].afterPriority
                });
            }
            if (eventnet_dev_1._attrsStore.normal[name].finish) {
                this._attrs.finishSequence.push({
                    name,
                    value: attr[name],
                    priority: eventnet_dev_1._attrsStore.normal[name].finishPriority
                });
            }
        }
        ...
        if (this._attrs.own.sync) {
            try {
                ...
                if (typeof error.when !== "undefined") {
                    ...
                    --this.state.running;
                    this.errorHandler(types_1.NodeRunningStage.code, error);
                }
            }
        }
        else {
    ...
    async _codeAsync(data, caller) {
        ...
        for (const attrObj of this._attrs.beforeSequence) {
            await eventnet_dev_1._attrsStore.normal[attrObj.name].before(attrObj.value, conditionBefore);
            if (shutByAttrBefore) {
        ...
        if (this._attrs.finishSequence.length !== 0) {
            ...
            for (const attrObj of this._attrs.finishSequence) {
                await eventnet_dev_1._attrsStore.normal[attrObj.name].finish(attrObj.value, conditionFinish);
                if (shutByAttrFinish) {
                    runningStage = types_1.NodeRunningStage.over;
                    --this.state.running;
    ...
    _codeSync(data, caller) {
        ...
        for (const attrObj of this._attrs.beforeSequence) {
            eventnet_dev_1._attrsStore.normal[attrObj.name].before(attrObj.value, conditionBefore);
            if (shutByAttrBefore) {
                runningStage = types_1.NodeRunningStage.over;
                --this.state.running;
                throw { when: types_1.NodeRunningStage.before, what: errorInAttrBefore };
        ...
        if (this._attrs.finishSequence.length !== 0) {
            ...
            for (const attrObj of this._attrs.finishSequence) {
                eventnet_dev_1._attrsStore.normal[attrObj.name].finish(attrObj.value, conditionFinish);
                if (shutByAttrFinish) {
                    runningStage = types_1.NodeRunningStage.over;
                    --this.state.running;
                    throw { when: types_1.NodeRunningStage.finish, what: errorInAttrFinish };
    ...
    codeDwsDataAttrAfterProcess(data, collection) {
        ...
        for (const attrObj of this._attrs.afterSequence) {
            eventnet_dev_1._attrsStore.normal[attrObj.name].after(attrObj.value, condition);
            if (shutByAttrAfter) {
                this.state.running--;
    ...
    createArrow(node, options) {
        return {};
    }
    createPipe(node, options) {
        return {};
    }
    createTwpipe(node, options) {
        return {};
    }
    arrow(node, options) {
    ...
    alsoArrow(node, options) {
        this.createArrow(node, options);
        return this;
    }
    alsoPipe(node, options) {
        this.createPipe(node, options);
        return this;
    }
    alsoTwpipe(node, options) {
        this.createTwpipe(node, options);
        return this;
    }
}
